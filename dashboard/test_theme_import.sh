...
await test('paste handler applies immediately, updates terminal and persists; reload restores',async()=>{
 const p=await page(),t=theme();p.ids.themeJSON.value=JSON.stringify(t);p.ids.themeImport.listeners.click();assert.equal(p.root.dataset.theme,t.id);assert.equal(p.root.style['--bg'],'#123456');assert.equal(p.term.options.theme.background,'#123456');assert.deepEqual(JSON.parse(p.storage['ccc.importedThemes']),[t]);assert.equal(p.ids.themeSelect.children.at(-1).textContent,t.name);
...
await test('built-in export round-trips unchanged without duplicating or persisting as imported',async()=>{const p=await page();p.c.exportTheme();const exported=await p.blobs[0].text();assert.equal(p.c.importTheme(exported),true);p.c.exportTheme();assert.equal(await p.blobs[1].text(),exported);assert.equal(p.ids.themeSelect.children.length,data.themes.length);assert.deepEqual(JSON.parse(p.storage['ccc.importedThemes']),[]);});
...
await test('corrupt saved JSON leaves built-ins usable',async()=>{const p=await page({'ccc.importedThemes':'{'});assert.equal(p.root.dataset.theme,data.default);assert.match(p.ids.themeImportStatus.textContent,/Could not restore/);});
await test('stored unsafe or colliding themes rejected before swatches; valid entries recover',async()=>{const bad=theme('unsafe');bad.tokens['--bg']='url(//host)';const p=await page({'ccc.importedThemes':JSON.stringify([bad,theme(data.default),theme('good')]),'ccc.theme':'good'});assert.equal(p.root.dataset.theme,'good');assert.equal(p.ids.themeSelect.children.length,data.themes.length+1);assert.match(p.ids.themeImportStatus.textContent,/--bg/);assert.match(p.ids.themeImportStatus.textContent,/built-in/);});
