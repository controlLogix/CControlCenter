...
### 1. The flake itself — NOT fixed

`test_e2e.sh` failed once and passed on re-run and standalone. I do not know why, and that is the honest state: the evidence was destroyed by problem 2 before I could read it.

What is known: it starts its own server on an ephemeral port, a stub broker, and drives Playwright/Firefox. The gate runs it after ~55 other suites, so it launches a browser under whatever load and memory pressure that leaves. Candidates, none confirmed: browser launch timing out under load, the 120s per-test ceiling at `test_e2e.mjs:43`, or resource exhaustion.

It is the same family as TM-027 — a gate that gives two verdicts for one tree — but a different cause, since the gate runs from ext4 and this is not 9p EIO.

**This also qualifies TM-027's AC 5.** I demonstrated "the same verdict on two consecutive runs" on `de78285` and closed it. That demonstration was real but tree-specific; it is not a general guarantee while this exists.
...
### New: `dashboard/test_gate_reporting.sh`
...
**Next occurrence will be diagnosable**, which is the most that can be claimed until it happens again.